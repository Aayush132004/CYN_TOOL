...
import React, { useState, useEffect, useCallback } from 'react';
// ✅ FIX #2: Imported new icons for the detail modal.
import { Clock, Plus, X, Shield, Users, AlertTriangle, FileText, Trash2, Loader, ListChecks, Smartphone, Laptop } from 'lucide-react';
import axios from 'axios';
        ...
        fontFamily: "'JetBrains Mono', 'Fira Code', monospace",
                ...
                <div className="relative z-10">
                    ...
                    <main className="max-w-7xl mx-auto px-6 py-8">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                            <StatCard icon={FileText} label="Total Groups" value={groups.length} valueColor="text-[#00ff41]" borderColor="#00ff41" />
                            {/* ✅ FIX #3: Changed "red-500" to its hex code for Tailwind JIT compatibility. */}
                            <StatCard icon={AlertTriangle} label="Suspicious Total" value={groups.reduce((s, g) => s + g.suspiciousUsers, 0)} valueColor="text-[#ff1744]" borderColor="#ef4444" />
                            <StatCard icon={Users} label="Normal Total" value={groups.reduce((s, g) => s + g.normalUsers, 0)} valueColor="text-[#00ff41]" borderColor="#00ff41" />
                        </div>
                        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
                            {groups.map((group, index) => (<GroupCard key={group.id} group={group} index={index} onDelete={openDeleteModal} onViewDetails={openDetailModal} />))}
                ...
                {/* ✅ ENHANCEMENT: Upgraded detail modal to show all API data. */}
                <Modal isOpen={isDetailModalOpen} onClose={closeDetailModal}>
                    {selectedGroup && (
                        <div>
                            <div className="p-6 border-b border-[#1a1a1a] flex justify-between items-center">
                                        ...
                                        <tbody>
                                            {selectedGroup.suspiciousUserDetails.map((user) => (
                                                <tr key={user.user_id} className="border-b border-[#1a1a1a] hover:bg-white/5">
                                                    ...
                                                    <td className="p-3 text-neutral-400 font-mono">{user.last_known_ip}</td>
                                                    <td className="p-3 text-neutral-300 capitalize flex items-center gap-2">
                                                        {user.last_device_logged && (user.last_device_logged.toLowerCase() === 'android' || user.last_device_logged.toLowerCase() === 'ios') ? 
                                                            <Smartphone className="w-4 h-4 text-neutral-500" /> : 
                                                            <Laptop className="w-4 h-4 text-neutral-500" />}
                                                        {user.last_device_logged}
                                                    </td>
