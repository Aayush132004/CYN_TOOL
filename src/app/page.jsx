...
// import { Clock, Plus, X, Shield, Users, AlertTriangle, FileText, Trash2 } from 'lucide-react';
...
// const GroupCard = ({ group, index, onDelete }) => (
//   <div className="group relative bg-gradient-to-br from-[#0a0a0a] to-[#111111] rounded-2xl border border-[#1a1a1a] transition-all duration-500 hover:border-[#00ff41] hover:shadow-2xl hover:shadow-[#00ff41]/20 animate-fade-in overflow-hidden hover:-translate-y-1 hover:scale-[1.02]" style={{ animationDelay: `${index * 100}ms` }}>
...
//       <button onClick={() => onDelete(group.id)} className="text-neutral-600 hover:text-red-500 transition-all duration-200 opacity-0 group-hover:opacity-100" aria-label={`Delete group ${group.name}`} title="Delete Group">
...
//             <span>{Math.round((group.suspiciousUsers / (group.suspiciousUsers + group.normalUsers)) * 100)}%</span>
//           </div>
//           <div className="w-full bg-[#1a1a1a] rounded-full h-2 overflow-hidden">
//             <div className="h-full bg-gradient-to-r from-yellow-500 via-orange-600 to-red-600 transition-all duration-1000" style={{ width: `${(group.suspiciousUsers / (group.suspiciousUsers + group.normalUsers)) * 100}%` }}></div>
//           </div>
//         </div>
//       </div>
//     </div>
...
//         <div className="bg-gradient-to-br from-[#0a0a0a] to-[#111111] rounded-2xl w-full max-w-lg border border-[#1a1a1a] shadow-2xl shadow-[#00ff41]/10 animate-modal-in">
//           {children}
//         </div>
//       </div>
//     </>
...
//     { id: 1, name: 'Threat_Analysis_Alpha', suspiciousUsers: 15, normalUsers: 285, status: 'active', lastUpdate: '2 mins ago' },
//     { id: 2, name: 'Regional_Security_Monitor', suspiciousUsers: 8, normalUsers: 412, status: 'monitoring', lastUpdate: '5 mins ago' }
//   ]);
//   const [formData, setFormData] = useState({ groupName: '', metadataFile: null, suspiciousUsers: '' });

//   const closeFormModal = useCallback(() => { setIsFormModalOpen(false); setFormData({ groupName: '', metadataFile: null, suspiciousUsers: '' }); }, []);
...
//   //   const res = await axios.post("https://hack-fe0t.onrender.com/predict", formData.metadataFile , {
//   //     headers: {
...
//   // }
//   //   const newGroup = { id: Date.now(), name: formData.groupName, suspiciousUsers: parseInt(formData.suspiciousUsers, 10) || 0, normalUsers: Math.floor(Math.random() * 500) + 100, status: 'active', lastUpdate: 'just now' };
...
//     reader.onload = async () => {
//       try {
//         const jsonContent = JSON.parse(reader.result); // ✅ parsed JSON
//         console.log("📂 JSON Data:", jsonContent);

//         // Send JSON to backend API
//         const res = await axios.post(
//           "https://hack-fe0t.onrender.com/predict",
//           jsonContent,
//           {
//             headers: {
//               "Content-Type": "application/json",
...
//         // Update local state (example new group)
//         const newGroup = {
//           id: Date.now(),
//           name: formData.groupName,
//           suspiciousUsers:
//             parseInt(formData.suspiciousUsers, 10) || 0,
...
//         closeFormModal();
//       } catch (err) {
//         console.error("❌ Error parsing JSON or sending:", err);
//       }
//     };

//     reader.readAsText(formData.metadataFile); // ✅ read file content as text
//   } catch (error) {
//     console.error("❌ Error:", error);
//   }
// };
...
//   }, [isFormModalOpen, isDeleteModalOpen, closeFormModal]);

//   return (
//     <>
//       <div className="min-h-screen bg-black text-[#e0e0e0] relative overflow-hidden" style={{ fontFamily: "'JetBrains Mono', 'Fira Code', monospace", backgroundImage: `url('/Background[1].png')`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: 'fixed' }}>
        
//         {/* STEP 1: TEST WITH THIS LINE COMMENTED OUT. If the image appears, the path is correct. */}
...
//               {groups.map((group, index) => (<GroupCard key={group.id} group={group} index={index} onDelete={openDeleteModal} />))}
//             </div>
//             {groups.length === 0 && (
...
//             </div>
//             <div>
//               <label htmlFor="suspiciousUsers" className="block text-sm font-semibold text-[#ff1744] mb-2">SUSPICIOUS_COUNT</label>
//               <input type="number" id="suspiciousUsers" name="suspiciousUsers" value={formData.suspiciousUsers} onChange={handleInputChange} min="0" required className="w-full bg-black/50 border-2 border-[#1a1a1a] rounded-xl px-4 py-3 text-[#e0e0e0] focus:outline-none focus:border-[#ff1744] transition-colors" placeholder="127" />
//             </div>
...
const Home = () => {
    ...
    const [groups, setGroups] = useState([
        { id: 1, name: 'Threat_Analysis_Alpha', suspiciousUsers: 15, normalUsers: 285, status: 'active', lastUpdate: '2 mins ago', suspiciousUserDetails: [] },
        { id: 2, name: 'Regional_Security_Monitor', suspiciousUsers: 8, normalUsers: 412, status: 'monitoring', lastUpdate: '5 mins ago', suspiciousUserDetails: [] }
    ]);
