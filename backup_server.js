...
app.post("/predict", async (req, res) => {
  ...
  python.on("close", async (code) => {
    ...
    try {
      ...
      if (anchor !== -1) {
        const candidate = stdout.slice(anchor);
        ...
        if (lastBrace !== -1) {
          ...
          try {
            ...
            return res.status(500).json({
              error: "Invalid JSON from Python (salvage failed)",
          ...
          return res.status(500).json({
            error: "Invalid JSON from Python (no closing brace)",
            raw_stdout: stdout,
            raw_stderr: stderr,
          });
        }
  ...
  try {
    ...
    const data = await Prediction.aggregate([
      // Stage 1: Group documents by 'GroupName'
      {
        $group: {
          _id: "$GroupName",
          // ✅ CORRECT: Get 'normal_count' directly from the first document in the group
    ...
    const TotalGroups = data.length;
    const SuspiciousTotal = data.reduce((sum, group) => sum + group.users.length, 0);
    // ✅ CORRECT: Sum the 'normal_count' from each group
    const NormalTotal = data.reduce((sum, group) => sum + group.normal_count, 0);
